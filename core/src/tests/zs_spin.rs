...
#[test]
fn ssd() {
    let board = Board::from_strs_with_piece(&ZSD, Bag::new(0), Piece::new(PieceType::S)).mirror();
    let expected = Board::from_strs(&[], Bag::new(0));
...
fn b2b_sst() {
    let indented_hang =
        Board::from_strs_with_piece(&B2B_ZST[0], Bag::new(0), Piece::new(PieceType::S)).mirror();

    let normal_hang =
        Board::from_strs_with_piece(&B2B_ZST[1], Bag::new(0), Piece::new(PieceType::S)).mirror();

    let roofless_kick =
        Board::from_strs_with_piece(&B2B_ZST[2], Bag::new(0), Piece::new(PieceType::S)).mirror();

    let double_hang =
        Board::from_strs_with_piece(&B2B_ZST[3], Bag::new(0), Piece::new(PieceType::S)).mirror();

    let last = Board::from_strs(&B2B_ZST[4], Bag::new(0)).mirror();
